...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-bold ${s.badge}`}>
      <span className={`w-1.5 h-1.5 flex-shrink-0 ${s.dot}`} />
      {s.label}
    </span>
  )
}
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {cotizaciones?.length ?? 0} cotización
            {(cotizaciones?.length ?? 0) !== 1 ? 'es' : ''} guardada
            {(cotizaciones?.length ?? 0) !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100">
                {cotizaciones.map((cot: Record<string, unknown>) => {
                  ...
                  const r = cot.result as Record<string, number>
                  return (
